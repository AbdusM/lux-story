...
test.describe('Settings Overlay Parity', () => {
  test('settings re-renders cleanly when resizing across breakpoint while open', async ({ page, freshGame: _freshGame }) => {
    ...
    await expect(settingsDialog).toContainText(
      /Keep audio, accessibility, and account tools close without pushing admin or report UI back into the story\./i
    )
    await expect(settingsDialog).toContainText(/Audio, accessibility, backups, and account controls\./i)
