...
type VerificationSaveState = {
  ...
  characters?: StoredCharacterState[]
}

test.describe('Phase 1 Verification', () => {
  test('should have freshGame fixture working', async ({ page, freshGame: _freshGame }) => {
  ...
  test('should have stable testid selectors', async ({ page, freshGame: _freshGame }) => {
  ...
  test('should have choice buttons with testid', async ({ page, freshGame: _freshGame }) => {
  ...
  test('should have journeyComplete fixture with developed patterns', async ({ page, journeyComplete: _journeyComplete }) => {
  ...
  test('should have withDemonstratedSkills fixture', async ({ page, withDemonstratedSkills: _withDemonstratedSkills }) => {
  ...
  test('should have withHighTrust fixture for Maya', async ({ page, withHighTrust: _withHighTrust }) => {
    ...
    const mayaState = await page.evaluate<StoredCharacterState | null>(() => {
      const saved = localStorage.getItem('grand-central-terminus-save')
      if (!saved) return null
      const state = JSON.parse(saved) as VerificationSaveState
      return state.characters?.find((c) => c.characterId === 'maya') || null
    })
