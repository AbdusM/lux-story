...
import { buildCanonicalSkillGapAnalysis } from '@/lib/canonical-career-analysis'
...
export function SkillGapVisualizer({ careerId, height = 300 }: SkillGapVisualizerProps) {
    ...
    const analysis = useMemo(() => {
        ...
        return buildCanonicalSkillGapAnalysis(careerId, skills)
    }, [careerId, skills])
    ...
    const chartData = [
        ...analysis.strengths.map(s => ({
            subject: s.skillName,
            A: Math.round(s.level * 100),
            B: 100,
            fullMark: 100
        })),
        ...analysis.gaps.map(g => ({
            subject: g.skillName,
            A: Math.round(g.currentLevel * 100),
            B: Math.round(g.requiredLevel * 100),
            fullMark: 100
        }))
    ]
