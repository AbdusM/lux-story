...
const PerformanceMetricsSchema = z.object({
  ...
  anxiety: z.number(),
  rushing: z.number(),
  lastCalculation: z.number(),
  performanceHistory: z.array(z.number()),
  choiceTimestamps: z.array(z.number()),
  regionExploration: z.array(z.string()),
  characterInteractions: z.array(z.string())
})
...
export class PerformanceSystem {
  ...
  private loadMetrics(): PerformanceMetrics | null {
    ...
    return safeStorage.getValidatedItem(this.STORAGE_KEY, PerformanceMetricsSchema)
  }

  private saveMetrics(): void {
    const toSave = {
      ...this.metrics,
      // Convert Sets to arrays for storage (if any were Sets, but schema enforces Array)
      regionExploration: this.metrics.regionExploration,
      characterInteractions: this.metrics.characterInteractions
    }
    safeStorage.setItem(this.STORAGE_KEY, JSON.stringify(toSave))
  }
