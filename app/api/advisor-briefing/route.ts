...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!briefingText) {
      logger.error('No text in Claude response', { operation: 'advisor-briefing' })
      return NextResponse.json(
    ...
    logger.error('Advisor briefing generation failed', { operation: 'advisor-briefing', error: (error as Error)?.message })
