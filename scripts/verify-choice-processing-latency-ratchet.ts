...
const BUDGET_MS = Number(process.env.CHOICE_PROCESSING_P95_BUDGET_MS || 400)
const MIN_SAMPLES = Number(process.env.CHOICE_PROCESSING_MIN_SAMPLES || 20)

const SAMPLES_PATH = path.join(REPO_ROOT, 'docs/qa/choice-processing-latency-samples.json')
const BASELINE_PATH = path.join(REPO_ROOT, 'docs/qa/choice-processing-latency-baseline.json')
const REPORT_PATH = path.join(REPO_ROOT, 'docs/qa/choice-processing-latency-report.json')

function fail(message: string): never {
  // eslint-disable-next-line no-console
  console.error(`\n[verify-choice-processing-latency] ${message}\n`)
  process.exit(1)
}
...
function extractProcessingTimes(payload: unknown): number[] {
  ...
  const fromSamples = samples
    .map((row) => {
      ...
      if (row && typeof row === 'object') {
        ...
        if (typeof r.processing_time_ms === 'number') return r.processing_time_ms
  ...
  const fromEvents = events
    .map((row) => {
      ...
      const processingTime = payloadObj?.processing_time_ms
      return typeof processingTime === 'number' ? processingTime : null
    })
    .filter((v): v is number => typeof v === 'number' && Number.isFinite(v) && v >= 0)
...
function main(): void {
  ...
  const processingTimes = extractProcessingTimes(sourcePayload)
  if (processingTimes.length < MIN_SAMPLES) {
    fail(
      `Insufficient latency samples: required >= ${MIN_SAMPLES}, got ${processingTimes.length}. ` +
      `Populate ${path.relative(REPO_ROOT, SAMPLES_PATH)} with choice_selected_result.processing_time_ms samples.`,
    )
  }

  const current = computeStats(processingTimes)

  if (writeBaseline) {
    const baseline: Baseline = {
      generated_at: new Date().toISOString(),
      notes: 'Baseline for choice processing latency p95 gate (regression-only + hard budget).',
      budget_ms: BUDGET_MS,
    ...
    // eslint-disable-next-line no-console
    console.log(`[verify-choice-processing-latency] baseline written: ${path.relative(REPO_ROOT, BASELINE_PATH)}`)
    return
  }
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[verify-choice-processing-latency] OK: samples=${current.sample_count}, p95=${current.p95_ms}ms, budget=${BUDGET_MS}ms. ` +
    `Report: ${path.relative(REPO_ROOT, REPORT_PATH)}`,
  )
}
